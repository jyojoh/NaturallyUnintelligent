...
def take_turn():
    ...
    move_string = "random2 " + str(next_moves[0]//9) + " " + str(next_moves[0] % 9)
...
if __name__ == '__main__':
    ...
    while not os.path.exists("end_game"):
        if("random2.go" and len(moves_taken) == 0 and os.path.exists("first_four_moves")):
            with open("first_four_moves", "r") as f:
                moves_taken = f.readlines()
                for val in moves_taken:
                    val_split = val.split()
                    if val_split[0] == "random2":
